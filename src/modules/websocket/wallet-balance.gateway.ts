...
export class WalletBalanceGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private readonly walletTokenMints: Map<string, string[]> = new Map();
  ...
  handleDisconnect(client: Socket) {
    ...
    this.subscribedWallets.forEach((clients, wallet) => {
      ...
      if (clients.size === 0) {
        ...
        this.walletTokenMints.delete(wallet);
      }
    });
  }

  @SubscribeMessage('subscribeBalance')
  async handleSubscribeBalance(client: Socket, data: { walletAddress: string; tokenMints?: string[] }) {
    const { walletAddress, tokenMints = [] } = data;
    ...
    try {
      ...
      // Lưu token mints cho wallet này
      this.walletTokenMints.set(walletAddress, tokenMints);

      // Lấy số dư ban đầu thông qua RPC
      const balances = await this.getTokenBalances(walletAddress, tokenMints);
        ...
        const logSubscriptionId = this.wsConnection.onLogs(
          ...
          async (logs) => {
            ...
            try {
              ...
              const walletTokenMints = this.walletTokenMints.get(walletAddress) || [];
              const newBalances = await this.getTokenBalances(walletAddress, walletTokenMints);
  ...
  private async getTokenBalances(walletAddress: string, tokenMints: string[] = []) {
    try {
      const publicKey = new PublicKey(walletAddress);
      ...
      // Khởi tạo số dư - mặc định chỉ có SOL
      ...
      for (const { account } of tokenAccounts.value) {
        ...
        // Chỉ lấy số dư của các token được yêu cầu
        if (tokenMints.includes(mintAddress)) {
          balances[mintAddress] = amount;
          // this.logger.log(`Tìm thấy số dư ${mintAddress}: ${amount}`);
        }
      }
  ...
  async broadcastBalanceUpdate(walletAddress: string, tokenMints: string[] = []) {
    const balances = await this.getTokenBalances(walletAddress, tokenMints);
